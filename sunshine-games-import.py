...
STEAMGRIDDB_API_KEY = "<STEAMGRIDDB_APIKEY>" #Make an account on steamgridDB, get the API key under account settings
...
def restart_sunshine():
    ...
    if not process_found:
        ...
        time.sleep(2)
        print("Attempting to restart Sunshine...")
        try:
            # You need to manually start the process again.
            # This is a generic approach. You may need to adjust the path
            # to the Sunshine executable if it's not in your PATH.
            subprocess.Popen(['sunshine'])
            print("Sunshine restarted successfully.")
